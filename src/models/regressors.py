...
# Helpers
# -------------------------
def gate_entropy(gate: torch.Tensor, eps: float = 1e-8, reduction: str = "mean") -> torch.Tensor:
    """
    gate: (B, C, L) after softmax over C or L (depends on your design)
    Returns entropy over channel-dim by default (dim=1).
    """
    ent = -(gate * torch.log(gate + eps)).sum(dim=1)  # (B, L)
    if reduction == "mean":
        return ent.mean()
    if reduction == "none":
        return ent
...
class TemperatureSoftmax(nn.Module):
    def __init__(self, dim: int = 1, temperature: float = 0.5):
        super().__init__()
        self.dim = dim
...
class ConvAgeRegressorConfig:
    ...
    length: int = 8                 # L, must satisfy C*L == in_dim
    kernel_sizes: Tuple[int, ...] = (3, 5)
    dropout: float = 0.1
    gate_softmax_dim: int = 1       # softmax over channel dim by default
...
class ConvAgeRegressor(nn.Module):
    ...
    def __init__(self, cfg: ConvAgeRegressorConfig):
        ...
        self.gate_conv = nn.Sequential(
            ...
            TemperatureSoftmax(dim=cfg.gate_softmax_dim, temperature=1.0),
        )
    ...
    def forward(self, z_age: torch.Tensor, return_gate_entropy: bool = False):
        ...
        if return_gate_entropy:
            ent = gate_entropy(gate, reduction="mean")
            return pred, ent
        return pred
# src/models/regressors.py
